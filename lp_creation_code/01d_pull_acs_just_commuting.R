...
################################################################################
# ACS 1-year: population age 15-64, 2005-2024
################################################################################
...
# B01001: male and female age 15-64
pop_vars <- c(
  setNames(paste0("B01001_", sprintf("%03d", 6:19)), paste0("male_", 6:19)),
  setNames(paste0("B01001_", sprintf("%03d", 30:43)), paste0("female_", 30:43))
)
...
for (y in years) {
  for (s in states) {
    ...
    dt <- tryCatch(
      ...
      error = function(e) {
        print(paste("Failed:", y, s, "|", conditionMessage(e)))
        NULL
      }
    )
    ...
    if (!is.null(dt)) {
      dt[, year := y]
      dt[, area_fips := as.integer(GEOID)]
      
      vars <- grep("^(male|female)_.*E$", names(dt), value = T)
      dt[, population_15_64 := rowSums(.SD, na.rm = T), .SDcols = vars]
      
      dt_list[[k]] <- dt[, .(area_fips, year, population_15_64)]
      k <- k + 1
...
population_15_64 <- rbindlist(dt_list, use.names = T, fill = T)

fwrite(population_15_64, paste0(path, "/acs/acs_1y_population_15_64.csv"))

print("ACS 1-year population age 15-64 pull complete.")