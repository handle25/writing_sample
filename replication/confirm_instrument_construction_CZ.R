...
crosswalk <- read_excel("cz00eqvv1.xls") |> 
  ...
  fmutate(
    area_fips = as.character(as.integer(fips)),
    state = as.integer(substr(fips, 1, 2))
  ) |>
  fgroup_by(area_fips, commuting_zone_id_2000) |>
  fsummarize(state = flast(state))

cw_s <- copy(crosswalk) |> 
  fgroup_by(commuting_zone_id_2000) |> 
  fsummarize(statefip = flast(state))

crosswalk <- merge(crosswalk, 
              cw_s, 
...
for (i in seq_along(years_qcew)) {
  ...
  qcew <- qcew |> fgroup_by(commuting_zone_id_2000 , industry_code, year) |> 
  ...
  # immediately collapse to area industry year level 
  qcew_naics6 <- qcew_naics6 |> fgroup_by(commuting_zone_id_2000 , industry_code, year) |> 
    fsummarize(total_annual_wages = fsum(total_annual_wages), 
               annual_avg_emplvl = fsum(annual_avg_emplvl), 
               annual_avg_estabs_count = fsum(annual_avg_estabs_count)) |> 
    ungroup() |> data.table()
...
for (y in c(2000, 2007, 2013)){
  ...
  plot <- ggplot(map_dt) +
    ...
    theme_void() 
  ggsave(paste0(path,"/../figures/w_d_sh_empl_mfg_",y,"_CZ", date, ".pdf"), 
         height = 6, width = 6)
  ...
  ggsave(paste0(path,"/../figures/w_d_sh_empl_mfg_",y,"_CZ_full", date, ".pdf"),
         height = 6, width = 6)
